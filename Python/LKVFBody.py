...
df_points=pd.read_excel('./Python/BodyVF.xlsx', index=False)
...
while(capture.isOpened()):
    ...
    if(int(frameid)==10):
        for p in newp:
            pppp=[p.item(0), p.item(1)]
            df2= pd.DataFrame({
                    'points10': pd.Series(pppp)
                })
            df_points=df_points.append(df2,ignore_index=True,sort=False)
            with pd.ExcelWriter('./Python/BodyVF.xlsx') as writer:
                    df_points.to_excel(writer, sheet_name='Points', index=False)
    if(int(frameid)==100):
        for p in newp:
            pppp=[p.item(0), p.item(1)]
            df2= pd.DataFrame({
                    'points100': pd.Series(pppp)
                })
            df_points=df_points.append(df2,ignore_index=True,sort=False)
            with pd.ExcelWriter('./Python/BodyVF.xlsx') as writer:
                    df_points.to_excel(writer, sheet_name='Points', index=False)
